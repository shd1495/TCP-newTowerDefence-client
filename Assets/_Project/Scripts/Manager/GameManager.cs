...
public class GameManager : MonoSingleton<GameManager>
{
    ...
    public Vector3 CalcRandomTowerPosition()
    {
        ...
        var position = roads1[rand].transform.localPosition + new Vector3(0, Util.Random(-100.0f, 100.0f));
        //usesTowerPositions.Add(rand);
        return position;
    }
    ...
    {
        var tower = towers.LastOrDefault();
        if (tower != null)
        {
    ...
    IEnumerator MultiGameLoop()
    {
        ...
        while (isGameStart)
        {
            ...
            float spawnInterval = initialGameState.MonsterSpawnInterval / 1000f;
    ...
    public void RemoveMonster(int monsterId)
    {
        ...
        if (targetMonster != null)
        {
            ...
            {
                if (monster != null && monsters.Contains(monster))  // 리스트에 있는지 확인
                {
                    // 인덱스 기반으로 삭제하니 에러가 생겨서 인스턴스를 삭제하게 변경
